...
import { crearSeparador, editarSeparador } from '@/app/(app)/cuadernos/actions'
...
interface NuevoSeparadorModalProps {
  open: boolean
  onClose: () => void
  cuadernoId: string
  separador?: Separador  // modo edición
}

export default function NuevoSeparadorModal({
  open,
  onClose,
  cuadernoId,
  separador,
}: NuevoSeparadorModalProps) {
  const [color, setColor] = useState(separador?.color ?? '#7c3aed')
  const [isPending, startTransition] = useTransition()
  const formRef = useRef<HTMLFormElement>(null)
  const editMode = !!separador
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      if (editMode && separador) {
        await editarSeparador(separador.id, cuadernoId, formData)
      } else {
        await crearSeparador(cuadernoId, formData)
      }
      onClose()
    })
  }
    ...
    >
      <div className="w-[380px] rounded-2xl bg-white p-8 shadow-[0_24px_60px_rgba(0,0,0,0.2)]">
        ...
        <h2 className="font-fraunces text-xl font-semibold text-brand-900">
          {editMode ? 'Editar separador' : 'Nuevo separador'}
        </h2>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="mt-5">
          ...
          <div className="flex justify-end gap-2.5">
            ...
            >
              {isPending ? 'Guardando...' : editMode ? 'Guardar cambios' : 'Crear separador'}
            </button>
          </div>
        </form>
      </div>
